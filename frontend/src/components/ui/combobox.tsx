...
const CheckIcon = () => (
  <svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="stroke-gray-1000">
...
export const Combobox = ({
  ...
  return (
    <ComboboxContext.Provider value={{
      isOpen, setIsOpen, placeholder, inputValue,
      onChangeInputValue, disabled, errored, size,
      value: _value, onChangeValue, options, setOptions, inputRef,
    }}>
      <div className="relative w-full inline-block text-sm font-sans" style={{ width }}>
        {children}
      </div>
...
const ComboboxList = ({
  ...
  useEffect(() => {
    const options = filteredChildren
      .map((child) => {
        if (React.isValidElement<ComboboxOptionProps>(child)) {
          return { value: child.props.value, label: child.props.children?.toString() || "" };
        }
        return undefined;
      })
      .filter((o): o is Option => o !== undefined && o.label !== "");
    context?.setOptions(options);
  }, [children]);

  useEffect(() => {
    const getPosition = () => {
      if (context?.isOpen && context.inputRef?.current && menuRef.current) {
        ...
        if (buttonRect.bottom + menuHeight <= viewportHeight) {
          setPosition({ top: buttonRect.height + 28 });
        } else if (buttonRect.top - menuHeight >= 0) {
          setPosition({ bottom: buttonRect.height + 28 });
        } else {
          setPosition({ top: buttonRect.height + 28 });
        }
      }
